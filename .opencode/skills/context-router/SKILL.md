...
| `prd_driven` | `workflow` | `orchestrator` |
| `bug_fix` | `orchestrator` (BugChecker mode) | — |
| `audit` | `orchestrator` (full audit mode) | — |
| `project_build` | `workflow` (if PRD.md exists) / `orchestrator` | — |
...
> **Special Rule:** If `PRD.md` exists in the workspace root, ALL routing bypasses normal intent detection and goes directly to `workflow` skill. This is a hard override — no exceptions.
...
```
0. PRD DETECTION (BEFORE ALL ELSE)
   └── Check: does PRD.md exist in workspace root?
   └── IF YES → IMMEDIATE ROUTE to skill("workflow")
   └── Bypass all intent detection. This is a HARD OVERRIDE.
   └── Context: "PRD.md detected. Full pipeline execution requested."
...
### With workflow:
- Forward: prompt + "PRD.md found. Execute full 19-phase pipeline from PRD analysis to production launch."
- Context hint: attach full PRD.md path, all available skills, project structure
- Special: This is a HARD OVERRIDE. All normal routing is bypassed. workflow takes full control.
- Note: After workflow activation, context-router enters passive mode until pipeline completes.
